...
int main(int argc, char **argv){
    ros::init(argc, argv, "follow_rmse");
    ros::NodeHandle n;
    ...
    std::string rmse_mode = argv[1];
    if (argc < 2){
        ROS_ERROR("Missing required command line arguments <command_mode> (pos or vel)");
        ros::shutdown();
        return -1;
    } else if (rmse_mode not_eq "vel" and rmse_mode not_eq "pos"){ // never knew you could write logical ops like this
        ROS_ERROR("Missing required command line arguments <command_mode> (pos or vel), given ");
        ros::shutdown();
        return  -1;
    }
